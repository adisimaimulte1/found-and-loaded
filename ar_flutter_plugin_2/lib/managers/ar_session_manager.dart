...
class ARSessionManager {
  ...
  Future<Matrix4?> getCameraPose() async {
    try {
      // Get the raw pose from the platform channel
      final result = (await _channel.invokeMethod<Map<Object?, Object?>>('getCameraPose', {}));

      final position = result?['position'] as Map<Object?, Object?>?;
      final rotation = result?['rotation'] as Map<Object?, Object?>?;

      final rx = position?['x'] as num?;
      final ry = position?['y'] as num?;
      final rz = position?['z'] as num?;

      final qx = rotation?['x'] as num?;
      final qy = rotation?['y'] as num?;
      final qz = rotation?['z'] as num?;
      final qw = rotation?['w'] as num?;

      final positionVector = Vector3(rx!.toDouble(), ry!.toDouble(), rz!.toDouble());
      final quaternion = Quaternion(qx!.toDouble(), qy!.toDouble(), qz!.toDouble(), qw!.toDouble());

      final rotationMatrix = Matrix3.zero();
      quaternion.copyRotationInto(rotationMatrix);

      final transform = Matrix4.identity()
        ..setRotation(rotationMatrix)
        ..setTranslation(positionVector);

      return transform;

    } catch (e) {
      print('❌ getCameraPose() failed: $e');
      return null;
    }
  }
